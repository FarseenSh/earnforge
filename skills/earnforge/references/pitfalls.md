...
23 pitfalls, all verified against the live API on Jul 25, 2026 across 609 vaults.
...
| 15 | Empty `underlyingTokens` — **obsolete**, 0 of 609 vaults | Crash on `[0]` | Guard retained against a synthesised case |
...
`minTvlUsd` returned all 609 vaults instead of 33 — a "$100M+ TVL" filter that
silently returns sub-$20k dust.

**`verificationStatus` is documented nowhere** and flags 56 of 609 vaults, mostly
