...
function makeVaultListResponse(
  vaults: Vault[],
...
function createMockSdk(overrides: Partial<EarnForge> = {}): EarnForge {
  return {
    vaults: {
      list: vi
        .fn<() => Promise<VaultListResponse>>()
        .mockResolvedValue(makeVaultListResponse([makeVault()])),
      listAll: vi.fn(),
...
describe('useVaults', () => {
  it('3. returns loading state initially', () => {
    const sdk = createMockSdk()
    ;(sdk.vaults.list as ReturnType<typeof vi.fn>).mockReturnValue(
      new Promise(() => {})
    ) // never resolves
    const wrapper = createWrapper(sdk)
  ...
  it('5. returns error on failure', async () => {
    const sdk = createMockSdk()
    ;(sdk.vaults.list as ReturnType<typeof vi.fn>).mockRejectedValue(
...
describe('useVault', () => {
  it('8. returns loading then vault data', async () => {
    const sdk = createMockSdk()
    const wrapper = createWrapper(sdk)
    const { result } = renderHook(() => useVault('test-usdc-vault'), {
      wrapper,
    })
  ...
  it('10. returns error on vault fetch failure', async () => {
    const sdk = createMockSdk()
    ;(sdk.vaults.get as ReturnType<typeof vi.fn>).mockRejectedValue(
      new Error('Not found')
    )
    const wrapper = createWrapper(sdk)
...
describe('useEarnTopYield', () => {
  it('11. returns top vaults', async () => {
    const topVaults = [
      makeVault({ slug: 'top-1' }),
      makeVault({ slug: 'top-2' }),
    ]
    const sdk = createMockSdk()
  ...
  it('13. handles error state', async () => {
    const sdk = createMockSdk()
    ;(sdk.vaults.top as ReturnType<typeof vi.fn>).mockRejectedValue(
      new Error('API error')
    )
    const wrapper = createWrapper(sdk)
...
describe('usePortfolio', () => {
      ...
      positions: [
        {
          chainId: 8453,
          protocolName: 'aave',
          asset: { address: '0x1', name: 'USDC', symbol: 'USDC', decimals: 6 },
          balanceUsd: '1000',
          balanceNative: '1000',
        },
      ],
  ...
  it('16. handles portfolio error', async () => {
    const sdk = createMockSdk()
    ;(sdk.portfolio.get as ReturnType<typeof vi.fn>).mockRejectedValue(
...
describe('useStrategy', () => {
  it('19. returns strategy config for a preset', () => {
    const sdk = createMockSdk()
    const wrapper = createWrapper(sdk)
    const { result } = renderHook(() => useStrategy('conservative'), {
      wrapper,
    })
...
describe('useSuggest', () => {
  ...
  it('23. does not fetch when amount is 0', () => {
    const sdk = createMockSdk()
    const wrapper = createWrapper(sdk)
    const { result } = renderHook(() => useSuggest({ amount: 0 }), { wrapper })
...
describe('useApyHistory', () => {
  it('24. fetches APY history', async () => {
    const sdk = createMockSdk()
    const wrapper = createWrapper(sdk)
    const { result } = renderHook(() => useApyHistory('0xVAULT', 8453), {
      wrapper,
    })
  ...
  it('25. does not fetch when address is undefined', () => {
    const sdk = createMockSdk()
    const wrapper = createWrapper(sdk)
    const { result } = renderHook(() => useApyHistory(undefined, 8453), {
      wrapper,
    })
  ...
  it('26. does not fetch when chainId is undefined', () => {
    const sdk = createMockSdk()
    const wrapper = createWrapper(sdk)
    const { result } = renderHook(() => useApyHistory('0xVAULT', undefined), {
      wrapper,
    })
