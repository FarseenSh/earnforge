...
import { STRATEGIES, getStrategy, type StrategyPreset } from '../src/strategies.js';
...
describe('Strategy presets', () => {
  ...
  it('all strategies have required fields', () => {
    const presets: StrategyPreset[] = ['conservative', 'max-apy', 'diversified', 'risk-adjusted'];
  ...
  it('conservative only includes known blue-chip protocols', () => {
    ...
    for (const p of s.filters.protocols!) {
      expect(['aave-v3', 'morpho-v1', 'euler-v2', 'pendle', 'maple']).toContain(p);
    }
  });
