...
describe('withRetry', () => {
  ...
  it('throws immediately on non-retryable error', async () => {
    const fn = vi.fn().mockRejectedValue(new EarnApiError('bad request', 400, '/test'));
    await expect(withRetry(fn, { maxRetries: 3, baseDelay: 10 })).rejects.toThrow('bad request');
    expect(fn).toHaveBeenCalledTimes(1);
  });

  it('gives up after maxRetries', async () => {
    const fn = vi.fn().mockRejectedValue(new EarnApiError('server error', 500, '/test'));
    await expect(withRetry(fn, { maxRetries: 2, baseDelay: 10, maxDelay: 20 })).rejects.toThrow('server error');
    expect(fn).toHaveBeenCalledTimes(3); // initial + 2 retries
  });

  it('respects custom maxRetries of 0', async () => {
    const fn = vi.fn().mockRejectedValue(new EarnApiError('error', 500, '/'));
    await expect(withRetry(fn, { maxRetries: 0, baseDelay: 10 })).rejects.toThrow();
    expect(fn).toHaveBeenCalledTimes(1);
  });
});
