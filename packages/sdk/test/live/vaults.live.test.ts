...
import { VaultSchema, VaultListResponseSchema, ChainListResponseSchema, ProtocolListResponseSchema } from '../../src/schemas/index.js';
...
describe('Live API — Vault List', () => {
  ...
  it('pagination works via nextCursor (Pitfall #6)', async () => {
    ...
    const page2 = await client.listVaults({ chainId: 8453, cursor: page1.nextCursor! });
...
describe('Live API — Single Vault', () => {
  it('fetches STEAKUSDC on Base by chainId + address', async () => {
    ...
    for (const candidate of response.data.slice(0, 10)) {
      try {
        const vault = await client.getVault(candidate.chainId, candidate.address);
...
describe('Live API — Risk Score', () => {
  ...
  it('stablecoin vaults tend to score higher', async () => {
    ...
    const nonStable = response.data.filter((v) => !v.tags.includes('stablecoin'));

    if (stable.length > 0 && nonStable.length > 0) {
      const avgStable = stable.reduce((s, v) => s + riskScore(v).score, 0) / stable.length;
      const avgNonStable = nonStable.reduce((s, v) => s + riskScore(v).score, 0) / nonStable.length;
