...
function mockClient(
  vaultOverrides: Array<Partial<typeof baseVault>> = [{}]
): EarnDataClient {
  let callIdx = 0
  return {
...
describe('watch', () => {
  ...
  it('yields apy-drop event when APY drops by threshold', async () => {
    const highApy = {
      analytics: {
        ...baseVault.analytics,
        apy: { ...baseVault.analytics.apy, total: 10 },
      },
    }
    const lowApy = {
      analytics: {
        ...baseVault.analytics,
        apy: { ...baseVault.analytics.apy, total: 5 },
      },
    }
    const client = mockClient([highApy, lowApy])
    const gen = watch(client, '8453-0xbeef', {
      interval: 10,
      apyDropPercent: 20,
      maxIterations: 3,
    })
  ...
  it('yields tvl-drop event when TVL drops by threshold', async () => {
    const highTvl = {
      analytics: { ...baseVault.analytics, tvl: { usd: '100000000' } },
    }
    const lowTvl = {
      analytics: { ...baseVault.analytics, tvl: { usd: '50000000' } },
    }
  ...
  it('does not yield apy-drop when previous APY is 0', async () => {
    const zeroApy = {
      analytics: {
        ...baseVault.analytics,
        apy: { ...baseVault.analytics.apy, total: 0 },
      },
    }
