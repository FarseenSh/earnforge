...
describe('CLI commands', () => {
  ...
  describe('quote', () => {
    it('outputs JSON quote', async () => {
      const output = await runCommand([
        'quote', '--vault', '8453-0xbeef0001', '--amount', '10', '--wallet', '0xwallet', '--json',
      ]);
      const parsed = JSON.parse(output);
    ...
    it('outputs human-readable quote by default', async () => {
      const output = await runCommand([
        'quote', '--vault', '8453-0xbeef0001', '--amount', '10', '--wallet', '0xwallet',
      ]);
    ...
    it('calls SDK with correct parameters', async () => {
      await runCommand([
        'quote', '--vault', '8453-0xbeef0001', '--amount', '10', '--wallet', '0xw', '--json',
      ]);
      expect(forge.vaults.get).toHaveBeenCalledWith('8453-0xbeef0001');
    ...
    it('supports --from-token override', async () => {
      await runCommand([
        'quote', '--vault', '8453-0xbeef0001', '--amount', '10',
        '--wallet', '0xw', '--from-token', '0xdai', '--json',
      ]);
      expect(forge.buildDepositQuote).toHaveBeenCalledWith(
        expect.anything(),
  ...
  describe('suggest', () => {
    it('outputs JSON allocations', async () => {
      const output = await runCommand([
        'suggest', '--amount', '10000', '--asset', 'USDC', '--json',
      ]);
      const parsed = JSON.parse(output);
    ...
    it('outputs table by default', async () => {
      const output = await runCommand([
        'suggest', '--amount', '10000', '--asset', 'USDC',
      ]);
    ...
    it('passes params to SDK', async () => {
      await runCommand([
        'suggest', '--amount', '5000', '--asset', 'WETH', '--max-chains', '3', '--json',
      ]);
    ...
    it('supports strategy preset', async () => {
      await runCommand([
        'suggest', '--amount', '10000', '--asset', 'USDC', '--strategy', 'conservative', '--json',
      ]);
      expect(forge.suggest).toHaveBeenCalledWith(
  ...
  describe('watch', () => {
    it('outputs JSON events', async () => {
      const output = await runCommand(['watch', '--vault', '8453-0xbeef0001', '--json']);
      const parsed = JSON.parse(output);
    ...
    it('passes threshold options', async () => {
      await runCommand([
        'watch', '--vault', '8453-0xbeef0001', '--apy-drop', '10', '--tvl-drop', '15', '--json',
      ]);
  ...
  describe('doctor', () => {
    it('outputs JSON report for a vault', async () => {
      const output = await runCommand(['doctor', '--vault', '8453-0xbeef0001', '--json']);
      const parsed = JSON.parse(output);
