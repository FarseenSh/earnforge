...
import { makeVault, makeNonTransactionalVault, makeHighRiskVault, MOCK_RISK_SCORE } from './fixtures.js';

function createEmptyForge(): EarnForge {
  return {
    vaults: {
      list: vi.fn().mockResolvedValue({ data: [], nextCursor: null, total: 0 }),
      listAll: vi.fn().mockImplementation(async function* () { /* empty */ }),
    ...
    suggest: vi.fn().mockResolvedValue({ totalAmount: 0, expectedApy: 0, allocations: [] }),
    optimizeGasRoutes: vi.fn().mockResolvedValue([]),
    watch: vi.fn().mockImplementation(async function* () { /* empty */ }),
...
describe('edge cases', () => {
  ...
  describe('empty result sets', () => {
    ...
    it('suggest --json returns empty allocations', async () => {
      const output = await runCommand(['suggest', '--amount', '1000', '--asset', 'USDC', '--json']);
      const parsed = JSON.parse(output);
  ...
  describe('risk score dimensions', () => {
    ...
    it('non-redeemable vault penalty is reflected', () => {
      ...
      expect(report.riskScore!.score).toBeLessThanOrEqual(healthyReport.riskScore!.score);
    });
  });
});
