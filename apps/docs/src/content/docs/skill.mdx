...
| `earnforge protocols [--json]` | List 27 protocols |
...
5. **Risk score thresholds:** >= 8 is low risk, 6-7.9 is medium, < 6 is high
   risk. A verification-flagged vault can never reach 8.

6. **`tvl.usd` is a number** (it was a string, and the OpenAPI spec still says
   string). Parse with `parseTvl()`, which accepts either.

7. **`apy1d`, `apy7d`, `apy30d`, `apy.base` and `apy.reward` can all be null.**
   Use the fallback chain. APY is already a percentage — LI.FI's own spec and
   quickstart say to multiply by 100, which overstates every yield 100x.
